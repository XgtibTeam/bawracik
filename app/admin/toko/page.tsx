...
function ProdukTab() {
  ...
  const [importCabangId, setImportCabangId] = useState('');
  ...
  useEffect(() => {
    fetch('/api/auth/me')
      ...
      .then((d) => {
        ...
        if (d.session?.cabangId) setImportCabangId(d.session.cabangId);
      });
    fetch('/api/branches')
      .then((r) => r.json())
  ...
  async function addProduct() {
    setAddMsg(null);
    if (!nama.trim()) {
      setAddMsg({ type: 'error', text: 'Nama parfum wajib diisi.' });
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setAddMsg({ type: 'error', text: data.error || 'Gagal menambah produk.' });
        return;
      }
      setAddMsg({ type: 'ok', text: `Produk "${data.product.nama}" (${data.product.kode}) berhasil ditambahkan.` });
      setNama('');
      setKode('');
      setKategori('');
      setDeskripsi('');
      ...
      setAddMsg({ type: 'error', text: err.message || 'Gagal menambah produk.' });
    } finally {
  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    if (!importCabangId) {
      setImportMsg('Pilih cabang dulu (dipakai kalau file ada kolom stok).');
      e.target.value = '';
      return;
    }
    setImporting(true);
    ...
    try {
      ...
      const res = await fetch('/api/products/import', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ rows, cabangId: importCabangId }),
      });
      const data = await res.json();
      if (!res.ok) {
      ...
      let msg = `Berhasil import ${data.imported} produk, dilewati ${data.skipped}.`;
      if (data.stokDiisi > 0) msg += ` Stok awal terisi untuk ${data.stokDiisi} produk.`;
      setImportMsg(msg);
      load();
    } finally {
      setImporting(false);
    ...
    <div className="space-y-4">
      <div className="ticket space-y-2 p-4">
        ...
        <p className="text-xs text-ink/50">
          ...
          Deskripsi, Harga jual, link gambar, Ukuran Botol, <b>stok</b> (dalam ml — atau isi kolom{' '}
          <b>stok_kg</b> kalau maunya dalam kg, otomatis dikonversi ×1000).
        </p>
        <div>
          <label className="mb-1 block text-xs font-medium text-ink/60">
            Cabang (tujuan stok awal, kalau file ada kolom stok)
          </label>
          <select
            value={importCabangId}
            onChange={(e) => setImportCabangId(e.target.value)}
            disabled={!isSuperadmin}
            className="w-full rounded-lg border border-ink/15 px-3 py-2 text-sm outline-none focus:border-accent disabled:bg-paper disabled:opacity-70"
          >
            <option value="">— Pilih cabang —</option>
            {branches.map((b) => (
              <option key={b.id} value={b.id}>
        ...
        <input type="file" accept=".xlsx,.xls,.csv" onChange={handleImportFile} disabled={importing} className="text-xs" />
      ...
      <div className="ticket space-y-2 p-4">
        ...
        <Field label="Kode (kosongkan untuk dibuat otomatis)" value={kode} onChange={setKode} />
        <div>
        ...
        {addMsg && (
          <p className={`text-xs ${addMsg.type === 'ok' ? 'text-accent' : 'text-danger'}`}>{addMsg.text}</p>
        )}
        <button
          onClick={addProduct}
...
function SusulanPanel({ cabangId }: { cabangId?: string }) {
  const [items, setItems] = useState<
    { id: string; tanggal: string; karyawan: string; totalMl: number; totalHarga: number; jumlahItem: number }[]
  >([]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/reports/susulan?${params.toString()}`)
      .then((r) => r.json())
  ...
  const totalHarga = items.reduce((s, it) => s + it.totalHarga, 0);

  return (
    <div className="ticket space-y-3 p-4">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-sm font-semibold text-ink">Transaksi Susulan</h2>
        <span className="text-xs font-semibold text-ink/60">Rp{totalHarga.toLocaleString('id-ID')}</span>
      </div>
      <p className="-mt-1 text-[11px] text-ink/40">
        Daftar checkout yang di-backdate lewat toggle "Input Susulan" di halaman kasir — karyawan lupa input di
        tanggal kejadiannya, baru diinput belakangan.
      </p>
      <div className="flex gap-2">
        <input
          type="date"
      ...
      {!loading && items.length === 0 && <p className="text-xs text-ink/40">Tidak ada transaksi susulan di periode ini.</p>}
      {!loading && items.length > 0 && (
        <ul className="divide-y divide-ink/10 text-sm">
          {items.map((it) => (
            <li key={it.id} className="flex items-center justify-between py-2">
              <div>
                <p className="text-ink">{it.karyawan}</p>
                <p className="text-[11px] text-ink/40">
                  {new Date(it.tanggal).toLocaleString('id-ID')} · {it.jumlahItem} item · {it.totalMl.toLocaleString('id-ID')} ml
                </p>
              </div>
              <span className="font-semibold text-ink">Rp{it.totalHarga.toLocaleString('id-ID')}</span>
            </li>
          ))}
        </ul>
      )}
...
function RekapTab() {
  const [session, setSession] = useState<Session | null>(null);
  const [branches, setBranches] = useState<Branch[]>([]);
  const [nestedTab, setNestedTab] = useState<'penjualan' | 'susulan'>('penjualan');
  ...
  if (nestedTab === 'susulan') {
    return (
      <div className="space-y-4">
        {nestedSwitcher}
        <SusulanPanel cabangId={isSuperadmin ? cabangId || undefined : session?.cabangId ?? undefined} />
      </div>
    );
  }
