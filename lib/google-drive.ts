...
export type DriveFolder = 'attendance' | 'products' | 'feed' | 'bukti';
...
function getFolderId(folder: DriveFolder = 'attendance'): string {
  ...
  // Bukti bayar QRIS (self-checkout) numpang di folder feed yang sudah ada
  // supaya tidak perlu tambah env var baru — sama-sama gambar publik.
  if (folder === 'bukti') return getEnv('GOOGLE_DRIVE_FOLDER_ID_FEED');
