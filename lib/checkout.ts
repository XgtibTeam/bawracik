...
export type FinalizeCheckoutInput = {
  ...
  // Input SUSULAN: kasir lupa mencatat penjualan di tanggal kejadiannya,
  // baru sempat input belakangan. `tanggal` (ISO datetime) di sini
  // menggantikan "now()" sebagai createdAt transaksi, supaya transaksi ini
  // tetap terhitung masuk rekap tanggal yang sebenarnya, bukan tanggal
  // dia baru input. Hanya boleh dipakai kasir/admin/superadmin yang login
  // (dicek di API route, bukan di sini).
  tanggal?: string;
  susulan?: boolean;
};
...
export async function finalizeCheckout(input: FinalizeCheckoutInput) {
  ...
  // Backdate hanya kalau `tanggal` valid & diminta lewat toggle "Input
  // Susulan" — kalau parsing gagal, jatuh balik ke waktu sebenarnya.
  const createdAt = input.tanggal && !isNaN(Date.parse(input.tanggal)) ? new Date(input.tanggal).toISOString() : nowIso;
